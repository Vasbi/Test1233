...
import { GridComponent, ColumnsDirective, ColumnDirective, Inject, Selection, Toolbar, Edit, CommandColumn, Filter, Sort, Page } from '@syncfusion/ej2-react-grids';
import { DialogComponent } from '@syncfusion/ej2-react-popups';
import { apiClient } from '@/lib/apiClient';
import { useProjectScheduleStore, useTaskLinkStore, useRiskStore } from '@/store/globalStore';
...
export default function TaskLinkingPanel({
  ...
  const { tasks, setTasks } = useProjectScheduleStore();
  ...
  useEffect(() => {
    const fetchTasksAndRisks = async () => {
      try {
        ...
        // Load tasks
        const tasksResponse = await apiClient({ method: 'GET', url: `/api/projects/${projectId}/tasks` });
        setTasks(tasksResponse.data);
        
        // Load risks
        const risksResponse = await apiClient({ method: 'GET', url: `/api/risks`, params: { projectId } });
        setRisks(risksResponse.data);
        
      } catch (err) {
        console.error("Error fetching tasks and risks:", err);
        setError("Failed to load tasks and risks");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      try {
        ...
        const response = await apiClient({ method: 'GET', url: `/api/projects/${projectId}/suggest-links` });
  ...
  const handleSaveSuggestions = async () => {
    ...
    try {
      ...
      toast({
        title: "Links saved",
        description: `Created ${response.data.length} task-risk links`
      });
  ...
  const handleDeleteLink = async (linkId: number) => {
    ...
    try {
      await apiClient({ method: 'DELETE', url: `/api/task-risk-links/${linkId}` });
  ...
  // Calculate stats
  const taskWithLinks = tasks.filter(task => 
    task.links && task.links.length > 0
  ).length;
  
  const risksWithLinks = risks.filter(risk => 
    risk.taskLinks && risk.taskLinks.length > 0
  ).length;
  
  const completedTasksWithLinks = tasks.filter(task => 
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {taskWithLinks} / {tasks.length}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {((taskWithLinks / (tasks.length || 1)) * 100).toFixed(0)}% of tasks are linked to risks
            </p>
            <Progress 
              value={(taskWithLinks / (tasks.length || 1)) * 100} 
              className="mt-2" 
            />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {risksWithLinks} / {risks.length}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {((risksWithLinks / (risks.length || 1)) * 100).toFixed(0)}% of risks are linked to tasks
            </p>
            <Progress 
              value={(risksWithLinks / (risks.length || 1)) * 100} 
              className="mt-2" 
            />
          </CardContent>
        </Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              {/* TODO: Replace legacy table and dialog UI with Syncfusion DataGrid and Dialog for suggestions and batch actions. */}
              <div className="rounded-md border">
                ...
                <div className="divide-y">
                  {suggestions.map((suggestion, idx) => {
                    ...
                    const risk = risks.find(r => r.id === suggestion.riskId);
      ...
      <Card>
        ...
        <CardContent>
          {tasks.filter(task => task.links && task.links.length > 0).length === 0 ? (
            <div className="text-center p-8 text-muted-foreground">
