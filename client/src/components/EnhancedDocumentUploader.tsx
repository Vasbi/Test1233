...
export const EnhancedDocumentUploader: React.FC<EnhancedDocumentUploaderProps> = ({ 
  ...
  const handleUploadFile = async (index: number) => {
    ...
    setFiles(files.map((f, i) => i === index ? { ...f, status: 'uploading', progress: 0 } : f));
    ...
    try {
      ...
      setFiles(files.map((f, i) => i === index ? { ...f, status: 'success', progress: 100, uploadedId: data.id } : f));
      ...
      setFiles(files.map((f, i) => i === index ? { 
        ...f, 
        status: 'error', 
        progress: 0,
  ...
  const analyseDocuments = async () => {
    ...
    try {
      ...
      const response = await fetch('/document-upload/analyse', {
        method: 'POST',
      ...
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="files" className="mt-4 space-y-4">
            ...
            {files.filter(upload => upload.status === 'success').length > 0 && (
            ...
            {files.some(f => f.status === 'success') && (
              <div className="mt-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    onClick={analyseDocuments}
                    disabled={isAnalyzing || !files.some(f => f.status === 'success')}
                  >
                    {isAnalyzing ? (
                      <>
