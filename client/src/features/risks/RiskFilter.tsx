// Filter component for risks (migrated from components/RiskFilter.tsx)
...
const RiskFilter = ({ risks, onFilterChange, onCreateRegister }: RiskFilterProps) => {
  ...
  // Extract unique values for filters (fix for ES5/TS downlevel iteration)
  function unique(arr: (string | undefined)[]) {
  ...
  const categories = unique(risks.map(risk => risk.riskCategory));
  const owners = unique(risks.map(risk => risk.ownedBy));
  const statuses = unique(risks.map(risk => risk.riskStatus));
  const registerTypes = unique(risks.map(risk => risk.registerType || 'default'));
  const departments = unique(risks.map(risk => risk.department || 'default'));
        ...
        <div>
          ...
          <Select value={registerType} onValueChange={setRegisterType}>
            ...
            <SelectContent>
              ...
              {/* When mapping for SelectItem, filter out undefined and cast to string */}
              {/* Register Types */}
              {registerTypes.filter((type): type is string => typeof type === 'string').map(type => (
        ...
        <div>
          ...
          <Select value={department} onValueChange={setDepartment}>
            ...
            <SelectContent>
              ...
              {/* Departments */}
              {departments.filter((dept): dept is string => typeof dept === 'string').map(dept => (
        ...
        <div>
          ...
          <Select value={owner} onValueChange={setOwner}>
            ...
            <SelectContent>
              ...
              {/* Owners */}
              {owners.filter((own): own is string => typeof own === 'string').map(own => (
        ...
        <div>
          ...
          <Select value={status} onValueChange={setStatus}>
            ...
            <SelectContent>
              ...
              {/* Statuses */}
              {statuses.filter((stat): stat is string => typeof stat === 'string').map(stat => (
